...
Use this script with a fresh desktop browser session at 1920×1080. Do not coach the player. Clear the route and restart the scene before each participant. The formative sample target is five first-time desktop players; this script records evidence but is not statistical release certification.

## Record

For each participant, note the browser and resolution, time to state the objective, time to first route attempt, completion time, route errors, recovery outcome, pause/focus behavior, and whether they choose to replay. Do not collect names or other personal information.
...
3. Let the player act. Observe whether they discover that the route begins at the coral source and ends at the blue receiver.
...
7. Use replay. Confirm the objective returns and the next player can begin from a known state.
8. Toggle sound cues and reduced motion. Confirm both settings are legible and have immediate effect.
